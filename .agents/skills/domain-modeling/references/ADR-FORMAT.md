...
```md
# ADR <issue> — {Short title of the decision}
...
- **Status** frontmatter (`proposed | accepted | deprecated | superseded by
  ADR <issue>`) — useful when decisions are revisited
...
Some repos have earlier ADRs as sequential `0001-slug.md`, `0002-slug.md`,
etc., with no issue id in the name. That format is legacy: keep those files
