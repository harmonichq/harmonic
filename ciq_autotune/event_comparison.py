...
def _validate_query(query: ComparisonQuery) -> tuple[dict, str]:
    if not isinstance(query, ComparisonQuery):
        raise ValueError("event comparison query must be a ComparisonQuery")
    config = VIEW_CONFIG.get(query.view)
    if config is None:
        raise ValueError("view must be meals or lows")
...
def _anchor(occurrence: dict, config: dict) -> dict:
    return {
        "kind": config["anchor_kind"],
        "t": occurrence["anchor_t"],
        "date": occurrence["date"],
        "bg": occurrence.get("anchor_bg"),
        "worst_bg": occurrence.get("worst_bg"),
        "label": config["anchor_label"],
    }


def _boundary_facts(route: dict) -> list[dict]:
    facts = []
    for key, value in route.get("boundary", {}).items():
        if not _finite(value):
            continue
        label, unit = _BOUNDARY_FACTS.get(key, (key.replace("_", " "), ""))
        facts.append({"key": key, "label": label, "value": value, "unit": unit})
    return facts


def _verdict(occurrence: dict, factor: str) -> dict:
    route = occurrence["routes"][factor]
    classifier = next(
        (item for item in occurrence.get("verdicts", []) if item["classifier"] == factor),
        {},
    )
    return {
        "factor": factor,
        ...
        "evidence_tier": classifier.get("evidence_tier"),
        "other_factors": [
            {"key": key, "label": FACTOR_LABELS[key]}
            for key in route.get("other_factors", [])
        ],
        "boundary_facts": _boundary_facts(route),
    }
...
@dataclass(frozen=True)
class EventComparisonPreparation:
    """Immutable source/classifier work with one projection interface."""

    _exposures: dict
    _catalog: dict

    @property
    def exposure_payload(self) -> dict:
        """A copy of #654's contract, isolated from the private catalog."""
        return deepcopy(self._exposures)

    def project(self, query: ComparisonQuery) -> dict:
        ...
        for occurrence in all_occurrences:
            counts[occurrence["routes"][factor]["cohort"]] += 1

        visible_keys = list(COHORT_ORDER)
        if query.another:
            visible_keys.append("another_factor")
        routed = {
            key: [
                occurrence for occurrence in all_occurrences
                if occurrence["routes"][factor]["cohort"] == key
            ]
            for key in visible_keys
        }
        selectable = [
            occurrence for key in visible_keys for occurrence in routed[key]
        ]
        occurrence_summaries = [
            _occurrence_summary(occurrence, config, factor)
            for occurrence in selectable
        ]
        selected = next(
            (occurrence for occurrence in selectable if occurrence["id"] == query.occurrence_id),
            None,
        )
        if query.occurrence_id is None:
            selection = {"state": "none", "detail": None}
        elif selected is None:
            selection = {
                "state": "unavailable", "requested_id": query.occurrence_id,
                "detail": None,
            }
        else:
            detail = _occurrence_summary(selected, config, factor)
            anchor_time = _dt(selected["anchor_t"])
            markers = []
            if query.view == "lows":
                for marker in selected["trace"].get("rescue_carbs", []):
                    grams = marker.get("grams")
                    if not _finite(grams) or grams <= 0:
                        continue
                    marker_time = marker.get("t") or (
                        anchor_time + timedelta(minutes=marker["minute"])
                    ).strftime(FMT)
                    markers.append({
                        "kind": "rescue_carb", "t": marker_time,
                        "minute": marker["minute"], "grams": grams,
                        "certainty": marker.get("certainty"),
                    })
            detail.update({
                "glucose": [
                    {"t": point.get("t") or (
                        anchor_time + timedelta(minutes=point["minute"])
                    ).strftime(FMT), "minute": point["minute"], "bg": point["bg"]}
                    for point in selected["trace"]["cgm"] if _finite(point.get("bg"))
                ],
                "markers": markers,
                "day_target": {"date": selected["date"]},
            })
            selection = {
                "state": "selected", "requested_id": query.occurrence_id,
                "detail": detail,
            }
        ...
        return {
            ...
            "coordinates": {
                ...
                "alignment_window_min": list(config["window"]),
                "factor_options": [
                    {"key": key, "label": FACTOR_LABELS[key]}
            ...
            "cohorts": [
                _cohort_projection(key, routed[key], config) for key in visible_keys
            ],
