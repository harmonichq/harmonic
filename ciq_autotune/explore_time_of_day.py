...
def build_time_of_day(store) -> dict:
    ...
    # Meals form over the whole bolus history, so which bolus opened a meal never
    # depends on where the window starts.
    history = store.bolus_events(None, format_t(end))
    boluses = [bolus for bolus in history if bolus.t >= start]
    meal_times = [meal.t for meal in group_meals(history) if meal.t >= start]
    for t in meal_times:
