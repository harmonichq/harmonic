...
case "${1:-}" in
  app)
    files="frontend/index.html frontend/theme.css frontend/diagnose-workstation.css frontend/verify-workstation.css harness/index.html harness/main.js scripts/screenshots.local.mjs"
    pattern='html\.dark|html:not\(\.dark\)|localStorage[^\n]*theme|cockpit-theme|prefers-color-scheme' ;;
  contracts)
