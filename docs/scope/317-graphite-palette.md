...
| 1c | 3 | Boundaries reconciled; Plan selector pinned; moved-token list by computed value; `--mk-accent` frozen | Reviewer verified all four; found the selector already in the cockpit gate's readiness map | COUNTERSIGN (panel 1) |
| 2 | 0 | Fresh cold pass (panel 2) on pin 1b897f6 | Reviewer: bar-move path collides with the cockpit gate's bar-equals-desk relationship assertion and its mutation, with no literal to re-base (`authoring`); palette rule admitted any colour-to-colour change without consulting the moved tokens, so a stray hardcoded literal passed as palette-only (`authoring`) | OBJECT (2 block, 0 note) |
| 2b | 2 | Bar-move gate shape recorded; palette rule takes a colour-pair table (13/13); third fail-closed mutation | Reviewer closed O2; O1 survives one assertion further down: the gate's distinct-grounds count, plus S6's opening vocabulary phrase and the gate's rationale comment (`injected`, the new no-other-structural-change clause turned an omission into a hard stop) | OBJECT (1 block, 0 note) |
| 2c | 1 | Bar-move gate shape completed (grounds count, S6 vocabulary, comment sweep) | Reviewer verified every anchor; note: two line-range starts off by 2 and 9 (mechanical, order only) | COUNTERSIGN (panel 2) |
| 3 | 0 | Fresh cold pass (panel 3, the cap) on pin 10b6b29 | Reviewer: a "three materials" paragraph the ticket body named does not exist in theme.css (`authoring`); a two-file `--ck-ground` edit cannot produce the ruled bar move (`authoring`); theme.css prose asserting the pre-ruling state has no sweep (`authoring`); the Trial opener has no base-side input (`injected`, from the round-1 B6 fix); runtime-only inline tokens (`--sev`) make the moved-token list unfillable by chunk 1 alone (`injected`, from the round-1 B2 rule). Note: no generated-facts appendix. All five fixed; cap reached | OBJECT (5 block, 1 note) |

Cap outcome: three panels; every blocker traced to grounding, none to an unsettled operator decision. The five cap findings are fixed at pin dfdf116 (see the lock) and were not re-reviewed by a fourth panel.
