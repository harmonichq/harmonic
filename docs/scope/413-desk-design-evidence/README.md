...
- **Captures** are synthetic only, named `<scene>-<before|after>-<size>.png`.
  - "Before" is the base.
  - "After" is the branch at c7fdad07, plus the evidence-only replay fixes
    that followed.
...
| Log | Checkout | Result |
|-----|----------|--------|
| `fail-first-base-1280x720.log` | base | S113–S117 all FAIL: 0 executed, 5 failed of 5 selected |
| `fail-first-base-1440x900.log` | base | the same at 1440x900 |
| `pass-branch-1280x720.log` | branch c7fdad07 | S113, S114, S115 and S117 PASS; S116 FAIL (see S116 below) |
| `pass-branch-1440x900.log` | branch c7fdad07 | the same at 1440x900 |

**S116.** Both c7fdad07 runs failed on base and branch alike with "at least
one ranked mini must be mounted".
- That was a defect in the story, not in the app. It looked for the ECharts
  instance on the canvas's parent div rather than on the `.mini` host.
- It also read texts off the option's graphic list rather than its one
  normalised group.
- 45e6aecc fixes the lookup. Its node test runs the story's own in-page
  function against that DOM shape and fails on the old lookup with the
  browser's message.
- **The coordinator still owes S116's rerun on base and branch at both sizes.**
  Those logs complete the fail-first and pass proof for S116.
...
**Inherited stories.** On the branch at c7fdad07 at 1280x720, these 18 executed
...
regressions its fixes introduced. The round-2 fixes are in 92ca1414. **Every
scene they touch still owes a re-capture**: `rail-arrival`, `mini-cause-event`,
`mini-pattern`, `urgency-one-tier`, `rail-first-pattern-open`,
`rail-fold-opened` (and `-item`) and `loading`, at both sizes.
...
| 4 | blocker | Unranked rows dropped their served sentence. | **Fixed** (c7fdad07); see the ruling above. Round 2 could not settle it from the captures: the captured unranked Patterns print no sentence. Theory: they serve none. The coordinator can confirm by reading `count_sentences` from the showcase's preparation. | node test |
| 5 | blocker | The Cause mini used colours opposite to the Pattern mini's. | **Fixed** (c7fdad07, ruling). One rail inking in the shared builder. Round 2: partly. Its claimed cohort (one case, all points withheld) draws no line, so the label keyed nothing. **Fixed** (92ca1414): a label whose line is not drawn steps down to the muted ink. | `mini-cause-event-after-*`, `mini-pattern-after-*` (re-capture owed); node tests |
| 6 | blocker | The anchor word came from a MEAL/LOW table. | **Fixed** (c7fdad07), from the served anchor label. Round 2: resolved ("LOW EXCURSION" matches the stage axis). New in round 2: the label collided with the dashed target line. **Fixed** (92ca1414): the label is knocked out on the mini's own ground. | `mini-*-after-*` (re-capture owed); node tests |
