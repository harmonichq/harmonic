...
- Round 3 (fresh cold Opus, no context): BLOCKED, 3 blocking + 2 notes, all `authoring`, all reproduced: the retained leave must be root.remove() only (leaveSurface resets the reading stack); the store read is half-open so the bound is [changed_at, end + 1 s) with an exact-end fixture reading; S112 needs a case with retained records (edit-chain); the pagehide arm keeps the full teardown (S84); the seated-navigation branch at diagnose.js:324 is dead and is deleted. First fix attempt aborted on a mismatched edit string and the ledger line was committed claiming the fixes (c2fcf5d9); the reviewer caught it (BLOCKED, 2). Second attempt landed tasks.md only (37b0c32d) and the reviewer found one more real defect: the entry-restoration observer must still disconnect on leave or the return repaint re-clicks a row. Third attempt applies design, spec, tasks 2.1 (observer disconnects) and the order body, each verified by grep before commit (f9709ec7). Re-check: one blocking (`seated` must stay true while detached and read completions must gate on root.isConnected, or an in-flight read restores against the detached root) and one note (pagehide unit case). Applied (24e54166). Re-check: one blocking (an off-screen completion advanced ages and payload without setData, so the return's revision check would confirm stale data current): the gate moves above the age and payload writes, off-screen completions are discarded and the next return re-reads. Applied (23f8e1eb). Re-check: one blocking (a bare root.isConnected gate breaks the cold open and Retry because root is null until the first payload): the discard gate is `seated && !root?.isConnected`, cold-open and Retry fail-first cases added. Applied (3d1d0109). Re-check: one blocking (a render during the moved-revision re-read re-seats the retained payload over the loading frame, because leave() never clears payload and every completion renders): mount gains a pending-read gate that shows the loading frame; fail-first case added. Applied (6194ea1b). Re-check: one blocking (the pending gate makes the discard gate unreachable; charter earned-guard rule): the discard gate is removed, off-screen completions are recorded and applied with restoration on the next return. Applied (25e05073). Re-check: one blocking (the same invariant makes the :65 and :68 seated-gated bodies dead): they are deleted, mount owns application; :52/:53 keep their gates. Order-only edit; same reviewer re-checks.
