...
| 3 re-check | 1 | Round-3 resolution pinned c795f16d | Blocker (`injected` by the round-3 fix): the retained-return re-read could resolve after an in-flight stage save and overwrite the Plan surface's copy with the pre-press draft, so the clearing re-seed dropped the 07:00 mark. Fixed: the destination keeps the stage callback's promise and skips the re-read while it is pending (ADR 460 point 7, task 5); task 7 gained the held-PUT, held-GET return test, proved on a broken variant; the surfaces delta gained its scenario. #459 CLEAN. | BLOCKED (1 block) |
