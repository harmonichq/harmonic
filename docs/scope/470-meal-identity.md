...
|---|---|---|---|---|
| 1 | — | Initial flat draft pinned 6dd05119 | Blockers: none. Notes (`authoring`): `explore_time_of_day.py` counts meals per bolus twice (`:55-57` bins, `:126-130` pooled), and task 75's test could pass on one; the budget leg (`acceptance.py budget`) was missing from Verification and Expectation; the base-relative replay selection is the complete ledger because tasks edit replay-global files; strict validation not runnable in the reviewer's sandbox. Refuted: none; each reproduced against the tree. Fixed: task 78 names both sites and task 75 asserts both fields; the budget leg is the suite run of record, and the Expectation judges whole pytest against the base on one machine (ADR 463); the lock's replay is an `ONLY=` selection of task 83's story at both sizes, and the complete ledger runs once on the final integration commit (coordinator). | CLEAN with 4 notes; fixed, no further panel by operator instruction |
