...
| Sequence findings are served coherently through existing finding interfaces (behavioral-layer, MODIFIED) | — | `tests/test_outcomes_trend.py` `SequenceTrendExclusionTest.test_supported_sequences_add_no_verify_behavior` (existing; it pins the `summarize_trend` `behaviors` roster the modified text now names) | — | — |
