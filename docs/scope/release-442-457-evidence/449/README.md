...
No requirement is removed. The Plan record and withdraw failure lines gained desk-level tests
at integration (`4b70a7f2`). Each presses the Plan's own control through `frontend/plan-view.js`'s
mount against a durable 409, and requires the line to read the headline followed by the served
sentence, with no code, status or "[object Object]". Each fails when the line prints the code
or the raw detail (shown on a scratch copy). The Plan line prints the client's typed error's
`message`, which carries the served sentence; it does not go through `failureMessage`, because
`frontend/follow-up.js` already imports `frontend/plan-view.js`.
