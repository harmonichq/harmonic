...
- **The switcher lists one entry per pump change, not per affected block.**
  ...
  at, and one change the user made is one entry. Disposition: -> ADR (recorded, ADR 136).
- **A multi-setting entry names the profile it moved to** ("Profile change ->
  ...
  destination profile, and its label is Q1b. Disposition: -> ADR (recorded, ADR 136).
...
- **An in-place multi-setting edit is not called a profile change at all.**
  ...
  Disposition: -> ADR (recorded, ADR 136).
...
- **Verify never names which setting moved the outcome.** Operator decision
  ...
  view per changed setting (basal on clean nights, ISF on rest windows, I:C on
  meal runs, per #134) and lets the reader judge. There
  ...
  causal claim the data does not support. Disposition: -> ADR (recorded, ADR 136).
...
- **Verify reuses Diagnose's interval language rather than inventing a second
  ...
  and keeps one uncertainty vocabulary across the app. Disposition: -> ADR (recorded, ADR 136).
...
- **A range that includes zero says "this data cannot tell yet" and names no
  ...
  naming a direction invents a result. Disposition: -> ADR (recorded, ADR 136).
- **Only the headline before-and-after numbers carry a range**, not every point
  ...
  all 48 clock bins reads as noise. Disposition: -> ADR (recorded, ADR 136).
...
- **The range counts days, not individual meals.** Operator decision (Q9,
  ...
  it is an upper bound. Disposition: -> ADR (recorded, ADR 136).
- **Decisions lock here; the build is handed to its own issues.** Operator
  ...
  build handoffs has lifted. Disposition: -> issue (#183, #182 filed).
...
- **Evidence views are one per affected part of the day, not one per setting.**
  ...
  that has an answer. Disposition: -> ADR (recorded, ADR 136).
...
- #183 filed: show one evidence view per affected part of the day inside a
  watched change, including ADR 136's three watched-change label forms:
  single-setting existing vocabulary, `Profile change → P0007` for an
  active-profile switch, and `Basal + carb ratio changed` for an in-place
  multi-setting edit.
- #182 filed: price Verify's outcome differences by day, withhold when the
  range spans zero, and use ADR 136's shared Diagnose interval wording.
