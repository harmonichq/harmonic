...
async function startForm(page, drill = false) {
  ...
  if (drill) {
    ...
    await waitForReplayAssertion(async seen => {
      assert.equal(seen(await page.locator('[data-start-focus]').count()), 0, 'background tile reads cannot offer a selected Focus');
    }, "startForm");
    await row.click();
...
async function preempted(page) {
  ...
  await waitForReplayAssertion(async seen => {
    assert.match(seen(await page.locator('[data-record-part="ending"]').innerText()), /preempted|does not resume/i);
    assert.equal(seen(await page.locator('[data-focus="pin"]').count()), 0);
  }, "preempted");
...
async function focusTables(page) {
  await waitForReplayAssertion(async seen => {
    const heads = seen(await page.locator('.gf-stage-focus table.gf-trend thead th:first-child').allTextContents());
    assert.equal(heads.length, 2); assert.match(heads[0], /observed behavior/i); assert.match(heads[1], /glucose outcomes/i);
    assert.doesNotMatch(seen(await page.locator('.gf-stage-focus').innerText()), /\bnull\b|\bundefined\b|\bNaN\b/);
  }, "focusTables");
}
...
export const C3_STORIES = {
  ...
  async S45(page) {
    const { detail } = await active(page);
    await waitForReplayAssertion(async seen => {
      assert.ok((seen(await page.locator('.gf-stage-trial .gf-title').innerText())).includes(detail.changes[0].slot));
      assert.ok(seen(await page.locator('[data-trial-chart] canvas').count()) > 0, 'the shipped hero rendered');
    }, "S45");
  },
  async S45b(page) {
  ...
  async S46(page) {
    ...
    await waitForReplayAssertion(async seen => {
      ...
      if (seen(await bar.count())) assert.ok(Number(seen(await bar.getAttribute('value'))) <= Number(seen(await bar.getAttribute('max'))));
      assert.doesNotMatch(seen(await page.locator('[data-part="maturity"]').innerText()), /15 of 14/);
    }, "S46");
  },
  async S47(page) {
    await active(page); await press(page, '[data-mode="daily"]');
    await waitForReplayAssertion(async seen => {
      assert.equal(seen(await page.locator('[data-mode="daily"]').getAttribute('aria-pressed')), 'true');
    }, "S47");
    await page.locator('[data-select="evidence-day"]').waitFor();
    await press(page, '[data-mode="summary"]');
    await waitForReplayAssertion(async seen => {
      assert.ok(seen(await page.locator('.gf-stage-trial table').count()) > 0);
    }, "S47");
  },
  async S48(page) {
    await active(page); await press(page, '[data-mode="daily"]');
    ...
    await waitForReplayAssertion(async seen => {
      ...
      assert.match(seen(await page.locator('.gf-day-read').innerText()), /not necessarily a complete day/);
    }, "S48");
  },
  async S49(page) {
    ...
    await waitForReplayAssertion(async seen => {
      const period = seen(await page.locator('[data-select="evidence-period"]').inputValue());
      const index = Number(seen(await page.locator('[data-select="evidence-day"]').inputValue()));
      const day = detail.day_rows[period][index];
      assert.ok(day, 'selected day must belong to the served period');
      const copy = seen(await page.locator('.gf-day-read').innerText());
      assert.equal(seen(await page.locator('.gf-day-read tbody tr:last-child .v').innerText()), String(day.meals));
      assert.doesNotMatch(copy, /\bnull\b|\bundefined\b|\bNaN\b/);
    }, "S49");
    await press(page, '[data-mode="summary"]');
    await waitForReplayAssertion(async seen => {
      const table = seen(await page.locator('[data-table="outcomes"]').innerText());
      ...
      for (const row of comparison.outcomes) {
        for (const [side, column] of [['before', 2], ['after', 3]]) {
          const cell = seen(await page.locator(`[data-outcome="${row.key}"] td:nth-child(${column})`).innerText());
          if (row[side] == null) {
  ...
  async S50(page) {
    await active(page);
    await waitForReplayAssertion(async seen => {
      const copy = seen(await page.locator('[data-trial-chart] .ds-chart-legend').innerText());
  ...
  async S51(page) {
    ...
    await page.fill('#conclusion', '   '); await waitForReplayAssertion(async seen => {
      assert.equal(seen(await submit.isDisabled()), true);
    }, "S51");
    await page.fill('#conclusion', 'Synthetic observation'); await waitForReplayAssertion(async seen => {
      assert.equal(seen(await submit.isDisabled()), false);
      assert.equal(seen(await page.locator('#conclusion').getAttribute('placeholder')), null);
    }, "S51");
  },
  ...
  async S53(page) {
    ...
    await waitForReplayAssertion(async seen => {
      assert.equal(seen(await page.locator('#conclusion').inputValue()), 'Synthetic retained conclusion');
      assert.equal((seen(await read(page, '/api/verify/trials', { selected: id }))).selected.original.ending.kind, undefined);
    }, "S53");
  ...
  async S54(page) {
    ...
    await waitForReplayAssertion(async seen => {
      const copy = seen(await page.locator('.gf-reading').innerText());
  ...
  async S57(page) {
    ...
    await waitForReplayAssertion(async seen => {
      assert.equal((seen(await read(page, '/api/verify/trials', { kind: 'focus', selected: id }))).selected.original.ending.kind, undefined);
      assert.equal(seen(await page.locator('#conclusion').inputValue()), 'Synthetic Focus observation');
    }, "S57");
    await page.unroute(path);
    ...
    await waitForReplayAssertion(async seen => {
      assert.equal(seen(await page.locator('[data-conclusion]').innerText()), 'Synthetic Focus observation');
    }, "S57");
  },
  ...
  async S59(page) {
    ...
    await waitForReplayAssertion(async seen => {
      assert.equal(seen(await page.locator('[data-focus^="seat-"]:visible').count()), 0);
      assert.equal(seen(await page.locator('.gf-stage-focus table.gf-trend:visible').count()), 2);
    }, "S59");
  },
  ...
  async S91(page) {
    ...
    await waitForReplayAssertion(async seen => {
      assert.equal(seen(await page.locator('[data-readiness="after"]').getAttribute('data-criterion-met')), 'false');
      assert.match(seen(await page.locator('[data-readiness="after"]').innerText()), /Synthetic served hold/);
      assert.equal(seen(await page.locator('[data-form="finish"]').count()), 1, 'finish permission is separate from evidence readiness');
    }, "S91");
  },
  async S92(page) {
    ...
    await waitForReplayAssertion(async seen => {
      ...
      assert.ok(seen(await page.locator('[data-trial-chart] canvas').count()) > 0);
    }, "S92");
    await ending(page);
    await waitForReplayAssertion(async seen => {
      assert.match(seen(await page.locator('[data-record-part="ending"]').innerText()), /unclear|no clear answer/i);
      assert.ok(seen(await page.locator('[data-trial-chart]').count()) > 0);
    }, "S92");
  },
  ...
  async S93(page) {
    ...
    await waitForReplayAssertion(async seen => {
      ...
      assert.equal(seen(await page.locator('[data-form="finish"]').count()), 1, 'readiness does not auto-end Focus');
    }, "S93");
  },
  async S94(page) {
    ...
    await waitForReplayAssertion(async seen => {
      assert.deepEqual((seen(await read(page, '/api/verify/trials', { selected: id }))).selected.original.ending, saved);
      assert.match(seen(await page.locator('.gf-reading').innerText()), /did not program|entered by hand/i);
    }, "S94");
  },
  async S95(page) {
  ...
  async S96(page) {
    ...
    for (const mode of ['retained', 'current']) {
      ...
      await waitForReplayAssertion(async seen => {
        assert.equal(seen(await page.locator('[data-conclusion]').innerText()), old.ending.conclusion);
        assert.deepEqual((seen(await read(page, '/api/verify/trials', { selected: old.id, assessment: mode }))).selected.original.ending, original.selected.original.ending);
      }, "S96");
    }
    await waitForReplayAssertion(async seen => {
      assert.match(seen(await page.locator('[data-record-part="original"]').innerText()), /Not recorded/);
    }, "S96");
  },
};
