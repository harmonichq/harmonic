...
const initialIssued = parseList('S01–S114, C41–C57, and D1–D3');
...
test('Diagnose behavior ledger rejects an issued ID without a replay story', () => {
  const ledgerWithOrphan = ledger
    .replace('**134 issued executable IDs:**', '**135 issued executable IDs:**')
    .replace('S01–S114', 'S01–S115');
...
test('Diagnose behavior ledger accepts a permanent retirement', () => {
  const retiredS91 = ledger
    .replace(
      ...
      '**Active executable IDs:** S01–S90, S92–S114, C41–C57, and D1–D3',
    )
...
test('Diagnose behavior ledger rejects coordinated deletion of an issued ID', () => {
  const deletedS91 = ledger
    .replace('**134 issued executable IDs:**', '**133 issued executable IDs:**')
    .replace('S01–S114', 'S01–S90, S92–S114');
  const withoutS91 = replay
    .replace("  ['S91', S91, 'drawn'],\n", '')
...
test('Diagnose behavior ledger rejects coordinated renumbering of an issued ID', () => {
  const renumberedS91 = ledger.replace('S01–S114', 'S01–S90, S92–S115');
  const replayWithS115 = replay
    .replace("  ['S91', S91,", "  ['S115', S115,")
    .replaceAll('// STORY:finding-evidence-routing:S91', '// STORY:finding-evidence-routing:S115');
  assert.throws(
    () => validate(renumberedS91, replayWithS115),
    /issued IDs cannot disappear or be renumbered/,
  );
});

test('Diagnose behavior ledger rejects a malformed story range', () => {
  const malformedRange = ledger.replace('S01–S114', 'S01–S114-S999');
