...
const initialIssued = parseList('S01–S112, C41–C57, and D1–D3');
...
test('Diagnose behavior ledger rejects an issued ID without a replay story', () => {
  const ledgerWithOrphan = ledger
    .replace('**132 issued executable IDs:**', '**133 issued executable IDs:**')
    .replace('S01–S112', 'S01–S113');
...
test('Diagnose behavior ledger accepts a permanent retirement', () => {
  const retiredS91 = ledger
    .replace(
      ...
      '**Active executable IDs:** S01–S90, S92–S112, C41–C57, and D1–D3',
    )
...
test('Diagnose behavior ledger rejects coordinated deletion of an issued ID', () => {
  const deletedS91 = ledger
    .replace('**132 issued executable IDs:**', '**131 issued executable IDs:**')
    .replace('S01–S112', 'S01–S90, S92–S112');
  const withoutS91 = replay
    .replace("  ['S91', S91, 'drawn'],\n", '')
...
test('Diagnose behavior ledger rejects coordinated renumbering of an issued ID', () => {
  const renumberedS91 = ledger.replace('S01–S112', 'S01–S90, S92–S113');
  const replayWithS113 = replay
    .replace("  ['S91', S91,", "  ['S113', S113,")
    .replaceAll('// STORY:finding-evidence-routing:S91', '// STORY:finding-evidence-routing:S113');
  assert.throws(
    () => validate(renumberedS91, replayWithS113),
    /issued IDs cannot disappear or be renumbered/,
  );
});

test('Diagnose behavior ledger rejects a malformed story range', () => {
  const malformedRange = ledger.replace('S01–S112', 'S01–S112-S999');
