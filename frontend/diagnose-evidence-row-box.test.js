// #31: the case-file roster's numeric cell is an inline cell, not a box.
//
...
// BOTH edges. The Diagnose case-file roster's anchor-glucose cell is a
// `<span class="only">` inside `.ev-row`, and `.ev-row .only` sets only ink
  ...
  for (const { name, css } of stylesheets) {
    ...
    for (const [, selectors, body] of flat.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
      for (const selector of selectors.split(',')) {
        ...
        if (!/\.only\b/.test(subject)) continue;
...
test('the case-file roster\'s numeric cell keeps its compact box (#31)', () => {
  assert.match(painter, /<span class="only">/,
    'the anchor-glucose cell is a span in the shipped case-file painter — if this '
  ...
  assert.deepEqual(owners.map((r) => `${r.stylesheet}: ${r.selector}`),
    ['diagnose-workstation.css: .ev-row .only'],
