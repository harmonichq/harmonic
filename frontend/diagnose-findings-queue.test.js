...
import {
  ...
  renderFindingsQueue,
  queueMeta, queueRows, presentedRows,
...
test('term 34 · settings and habits interleave in one list, ordered by the server', () => {
  ...
  assert.deepEqual(rows.map((r) => r.flavor),
    ['setting', 'setting', 'setting', 'habit', 'habit', 'habit', 'habit']);
  // the order is the projection's, untouched, over the rows the app presents
