...
export const S157 = async (page) => {
  for (const lever of ['high_carb_sequence', 'repeat_eating']) {
    ...
    if (lever === 'high_carb_sequence') {
      ...
      is(scoped.event.projection.response, input.windows.global.cases[id].event.projection.response,
        'clock roster retains the complete source response population');
      ok(scoped.event.occurrences.length < input.windows.global.cases[id].event.occurrences.length,
