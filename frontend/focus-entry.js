...
export function mount(host, deps = {}) {
  ...
  // A withheld Focus says why in the admission's own words (ADR 450), never
  // its served code; an unknown reason reads as their general sentence.
  const withheld = state.roster?.admission?.focus_pin?.reason;
  const copy = state.readFailure
    ...
    : e(withheld ? admissionReason(withheld).said : 'This Pattern is not currently offered as a Focus.');
