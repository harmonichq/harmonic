...
|---|---|---|---|---|
| Harmonic v2 desktop | Glucose-first workstation on a fixed dark desk: Overview leads with the decision, Explore owns the findings roster and evidence, Changes owns the Plan/Focus record and its ending, Day owns the chronology; served at `/v2/` alongside shipped v1 | `locked` | [#348](https://github.com/harmonichq/harmonic/issues/348) | locked mock: `mockups/harmonic-v2-glucose.html?source=journey` (+ `harmonic-v2-glucose.css`, `harmonic-v2-glucose.js` and its `-journey`/`-setting`/`-focus`/`-day`/`-utilities`/`-basal` modules) · lock manifest: `mockups/harmonic-v2-desktop.lock.md` · scaffold: `mockups/_theme.css` + `mockups/_shell.js` · generated inputs: `mockups/harmonic-v2.exploration/*.json` from `generate.py` · concept comparison and repair records: `mockups/harmonic-v2-review.html`, `mockups/harmonic-v2.exploration/{BRIEF,REVIEW,FABLE-REVIEW,COLD-WALKTHROUGHS,OPUS-QA-REPAIRS,AUDIT}.md` · historical variants reviewed in merged [#379](https://github.com/harmonichq/harmonic/pull/379) are retained as evidence: `mockups/harmonic-v2-guided.html`, `mockups/harmonic-v2-journal.html`, `mockups/harmonic-v2-chrome.html` · plan and decision records: `openspec/changes/harmonic-v2/` (ADR 348 ×7) · carried v1 jobs: `openspec/changes/harmonic-v2/predecessor.md`. Behavior ledger **★ FROZEN**: `mockups/harmonic-v2-desktop.behavior.md`, the executable half of this lock, with its fail-closed replay `frontend/harmonic-v2-desktop-behavior.replay.mjs` and its retained evidence `mockups/sweep/harmonic-v2-desktop/`. Replayed against the unchanged locked prototype at both locked viewports — 111 passed, 0 failed, 18 deferred, 129 selected — with one feature-specific negative proof per mock-applicable story (111 proved, 0 not proved) and 25 capture ids at both sizes. All 17 retirements keep their external sanctions and premises. **18 stories are app-opener-only and have never passed**: durable persistence and packaged delivery (`S86`–`S100`), a failed durable Trial finish (HV2-25), and heading focus on a full-width empty destination and on a Guide article (HV2-32). They are obligations on the build, and the app opener exits nonzero against the absent `/v2/` so an unbuilt surface cannot satisfy them silently. No QUESTION is open. The ledger plus the 34-term lock are the build contract; the surface stays **`locked`, not shipped** — built-app replay, fidelity pairs, the mock archive and the row's flip to `shipped` belong to the production delivery owner. V1 remains shipped and is not retired by this lock |
