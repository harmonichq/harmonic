...
render and the declared no-fetch server read a scratch copy of
`mockups/qa-e2e.synthetic/harmonic.sqlite`, generated in full by
`scripts/gen_qa_e2e_db.py`, through the one permitted offline serve
`AGENTS.md` declares (`--no-fetch --token ''` over that copy; corrected under
the #395 re-inventory amendment below). Both sources carry explicit synthetic
provenance and contain no real patient data.

## Re-inventory amendment — 2026-09-09, issue #395

Triage for #395 (the pattern row and per-pattern chart in the Diagnose rail)
replayed this ledger against the base
`codex/348-harmonic-v2-epic@2e7f5a0c35a74b33634775987b099f6f2a07c052` through
the QA copy-then-serve safe start on port 8765: `diagnose-workstation-behavior.replay.mjs`
passed 168 of 168 stories against the app and
`diagnose-event-comparison-behavior.replay.mjs` passed 14 of 14. A read-only
source re-inventory of `frontend/diagnose-findings-queue.js`,
`frontend/diagnose-workstation.js`, `frontend/diagnose-evidence-charts.js` and
`ciq_autotune/findings_projection.py` found every active story still present in
source; the apparent absences are the sanctioned retirements already recorded
below. No story is retired by this amendment and no executable ID is issued.

Two facts the diff surfaced are recorded here so the revision designs against
them rather than rediscovering them:

- The head-of-ledger live-render declaration named the retired
  `mockups/revise-e2e.synthetic/harmonic.sqlite` and its generator, neither of
  which exists in the tree (already noted by the #364 record below). That
  paragraph is a current-declaration line, not a historical run record, so it is
  corrected in place above to the QA copy-then-serve command `AGENTS.md`
  declares. Historical run records keep their original bytes.
- The findings projection now carries the backend-published `outcome_patterns`
  roster additively (issue #393; five keys, each with `readiness`, `k`/`n`,
  Wilson bounds, `settled_price`, `admission_route`, `collapse` and `members`).
  The shipped rail ignores it: `queueRows` and the single `paintRow` path read
  only the ranked rows, and no chart-registry descriptor exists for a pattern.
  On the base fixture the roster publishes three ready patterns and two
...
## Revision amendment — 2026-09-06, issue #364 (a case-level unavailable answer is not the window's failure)

A reader drilled into a Finding who then pressed a window preset that Finding
has no episodes in was left staring at `Findings unavailable for <scope>. Choose
another window to try again.` as the whole inspector, at every level. The
`Findings` breadcrumb popped the level without restoring the queue and Escape did
nothing; only pressing a preset again — even the same one — brought the queue
back, which is what proves the rows were there the whole time.

The trigger is the server's `404 finding_unavailable`, its documented normal
answer when the requested Finding has no inspectable member in the requested
window. The window change's preparation handshake carries that Finding's case
request as a shadow leg, and one `catch` covered both legs, so the case-level
answer was recorded as the *window's* findings failure and short-circuited the
queue root behind it.

**Behavior changed.** The shadow case leg now reads that one answer where it
arrives: it adopts the preparation the same chain already resolved as the
window's findings, pops the drilled frame back to the Findings queue, and clears
that frame's retained case file. No second request is issued and no reader-facing
copy is introduced. A genuinely failed preparation request is untouched and still
latches the window with the message above, as `C54` pins verbatim; a window
change into a window where the Finding *is* available still keeps the drill, as
`C55` pins.

**Behavior added.** The new executable story is:

```
C58 · Drilled into a Finding at 24 h, pressing a window preset whose case-file
      answer for that Finding is `404 finding_unavailable` lands on that
      window's Findings queue — breadcrumb `Findings`, no `Findings unavailable
      for …` in the level, the level settled — with the pressed preset still
      selected.
```

**Fail-first.** Against the unfixed product code with only this story added,
`ONLY=C58` reported `app: 0 of 1 stories passed` and named the latched
`Findings unavailable` state. That output is
`openspec/changes/364-window-change-latches-queue/evidence/replay.pre.stdout.txt`.
The complete passing run after the fix is `evidence/replay.post.stdout.txt` in
the same change, whose final line records the applicable count.

Both runs used the repo's one permitted offline serve — the copy-then-serve
`--no-fetch --token ''` entrypoint over a scratch copy of
`mockups/qa-e2e.synthetic/harmonic.sqlite`, generated by
`scripts/gen_qa_e2e_db.py`. The live-render prose near the head of this ledger
still names `mockups/revise-e2e.synthetic/harmonic.sqlite`; that path and its
generator no longer exist in the tree, and correcting frozen provenance is not
this revision's to do. Every input is synthetic; no real patient data was read.
No story is retired by this revision.
...
## Re-inventory amendment — 2026-09-10, issue #342 lock 2

The September 5 #342 baseline above remains historical evidence. This dated
re-inventory supersedes its current-authority status. The coordinator replayed
the inherited ledger against integrated base `08976444` through the declared
synthetic QA copy-then-serve on port 8767: **174 of 174 active stories passed**.
Complete output: session Scratch `s342/inventory/replay-c2base.log`.
The preceding pin `e013051021b98105c1eba75f5022b0ce0a838422` inventory is
`s342/inventory/inventory.md`; it records 174 rail and 14 event-comparison passes.
These are distinct runs. No new c3 rendered result is claimed by this amendment.

The current rail keeps #395's single painter, claimed-member tick, member count,
adjacency, rank exclusion and Findings/Sift exclusions. No inherited story is
amended or retired; S117 remains retired. No Pattern copy key is added.

The registry now has six chart families. Eating-sequence precedes the generic
event comparison for High-carb sequence and Repeat eating. Their canonical case
transport retains the prepared report and generation. The parent continues to
use pattern-case-file. The shared queue-mini mount, tile mount, All charts,
fullscreen shell, focus return and clock handling remain the shipped paths.
The sequence adapter owns two explicitly labeled unit panels (% and mg/dL),
served cohort names/counts, all three intervals and the served selected period.
It does not add an individual predicted glucose trace.

Source/style inventory: `frontend/diagnose-eating-sequences.js`,
`frontend/diagnose-evidence-charts.js`, `frontend/diagnose-workstation.js`;
shipped `frontend/index.html` tokens and event-comparison cohort aliases;
`harness/stories.js`, `harness/dev-server.js`, `harness/main.js`.
The existing Vite/Vue/ECharts runtime is retained. No dependency, polyfill,
stylesheet fork or new viewport behavior is introduced.

Dedicated manufactured input:
`mockups/eating-sequence-findings.synthetic/payload.json`, generated by
...
for reproducibility; admission, ownership, membership and report values are not
hand-set.

### New executable stories — S151–S158

- **S151:** High-carb sequence nests beneath Highs after meals in covered and
  structurally-empty winning states. It keeps its served sequence count and quiet
  tick, adds no rank or Findings/Sift count, and never enters rate_levers.
- **S152:** Repeat eating obeys S151's contract. When both causes are served,
  both stay adjacent below their parent in server order.
- **S153:** High-carb sequence's mounted queue mini and All charts cell use
  eating-sequence with served cohorts. The parent retains pattern-case-file.
- **S154:** Repeat eating uses that same chart family with three-plus versus
  single-window evidence at the served matched carb quintile.
- **S155:** High-carb sequence row pointer/keyboard activation and All charts
  selection drill to its canonical finding and coherent report. Fullscreen return
  preserves focus, selected sequence and clock window.
- **S156:** Repeat eating obeys the same drill/fullscreen/return contract.
- **S157:** Scoped queries omit whole-feed Patterns. Causes follow served outcome
  witnesses, retain source-window Priority, and acquire no orphan claimed_by.
- **S158:** Thin source states create no supported finding or zero substitute.
  Served null cells, selected periods, separate units, cohort counts and long
