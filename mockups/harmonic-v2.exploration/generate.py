...
def utilities_capture():
    ...
    source_text = (ROOT / DESK_GLOSSARY).read_text()
    start = "export const glossaryGroups = [\n"
    end = "];\n"
    ...
    literal = source_text.split(start, 1)[1].split(end, 1)[0]
    articles = {}
    ...
    line = source_text[:source_text.index(start)].count("\n") + 1
    module = ("// GENERATED by mockups/harmonic-v2.exploration/generate.py from "
              f"{DESK_GLOSSARY}:{line}.\nexport const glossaryGroups = [\n{literal}];\n")
    return {"_generated_by": "mockups/harmonic-v2.exploration/generate.py",
            "_note": "SYNTHETIC static source capture. Glossary wording is preserved verbatim from the desk's committed source; presentation labels remain governed by CONTEXT.md.",
            "guide": {"catalog": build_catalog(), "articles": articles},
            "glossary": {"source_path": DESK_GLOSSARY, "start_line": line,
...
def desk_material():
    """The desk's committed material, split into the app material and the Day legend.

    The legend rules ride in the same stylesheet the desk imports, and this
    exploration wants them apart: the material joins the combined app theme, the
    legend is its own sheet. The split is by rule prefix, the same bounded read
    the legend lift always used, and it fails closed rather than emit a legend
    sheet with no rules in it."""
    lines = (ROOT / DESK_MATERIAL).read_text().splitlines()
    legend = [line for line in lines if line.lstrip().startswith(".ds-chart-legend")]
    if not legend:
        raise RuntimeError(f"{DESK_MATERIAL} declares no .ds-chart-legend rules; extraction is unsafe")
    material = "\n".join(line for line in lines
                         if not line.lstrip().startswith(".ds-chart-legend"))
    return material.rstrip() + "\n", "\n".join(legend) + "\n"


def outputs():
    material, legend = desk_material()
    css = f"/* GENERATED by mockups/harmonic-v2.exploration/generate.py. Sources: {DESK_MATERIAL}, frontend/shell.css, frontend/theme.css. */\n"
    ...
    key_css = f"/* GENERATED by mockups/harmonic-v2.exploration/generate.py from the shipped Day legend in {DESK_MATERIAL}. */\n"
    key_css += legend
