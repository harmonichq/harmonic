...
on-chart legend SHALL keep its seat. A chart whose axis name already renders
inside its box SHALL keep the seating it has. The mini rank SHALL continue to
carry no axis name at all.
...
- **WHEN** the reader looks at the chart's axis names
- **THEN** the y-axis name reads `glucose change (mg/dL)` in full, with no part of
  it painted outside the chart
- **AND** the x-axis name reads `insulin acted (U)` in full, with no part of it
  painted outside the chart
- **AND** neither name overlaps the plotted correction steps

#### Scenario: The carb-ratio meal-runs chart states its elapsed-time unit

- **GIVEN** the Diagnose evidence canvas showing a carb ratio finding's meal runs
  at full rank
- **WHEN** the reader looks at the chart's x-axis name
- **THEN** the name reads `minutes from first meal` in full, with no part of it
  painted outside the chart
- **AND** a watched carb-ratio row and a ranked one seat that name identically

#### Scenario: Seating a name moves nothing that already rendered

- **WHEN** every finding row in the queue is opened in turn and each drawn axis
  name is measured against its own chart's bounds
