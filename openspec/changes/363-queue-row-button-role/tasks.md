...
- [ ] In `frontend/diagnose-findings-queue.js`, enclose each painted row button in
...
- [ ] Remove the `role` attribute from the row element itself. It stays a
...
- [ ] Give the wrapper of an unpriced tail row a marker the stylesheet can select,
      so the tail's own spacing rules can address it.
- [ ] Leave `role="list"` on the `.q` container, the rank numeral's `aria-hidden`,
...
- [ ] In `frontend/diagnose-workstation.css`, move the three adjacent-sibling
      ...
      (`:1712`).
- [ ] After the move the rules produce the same rendered result from a different
...
- [ ] Change no other queue rule. `.dw .qrow` and every descendant, attribute and
...
- [ ] Extend the queue test's DOM stub so `document.createElement` records the tag
...
- [ ] Add a test through `renderFindingsQueue` asserting that every painted row is
      ...
      that reason.
- [ ] Update the existing `#341` structure tests, which walk the container's
...
- [ ] Add one case to `frontend/diagnose-workstation.browser.test.mjs` that
...
- [ ] That case belongs in this suite, not in the behaviour replay: this file's
...
- [ ] Add exactly one story, `C60`, to `mockups/finding-evidence-routing.behavior.md`
...
- [ ] `C60`'s ledger text must carry the words `pending operator sanction at the
      #350 sweep PR`. This sweep has no attended operator, and the sweep's single
      pull request is where the operator sanctions it.
- [ ] Update the ledger's three inventory lines (`:14-16`) to match: the issued
...
- [ ] The replay reports `app: 164 of 164 stories passed` once `C60` is
      registered; it reports `163 of 163` at the base commit.
- [ ] Amend no existing story, retire nothing, and change no other ledger line.
