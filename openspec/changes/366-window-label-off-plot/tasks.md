...
- [x] Add one test to `frontend/diagnose-workstation-chart.test.js`, named for
...
- [x] Derive the injected `range` inside the test by calling the shipped
      ...
      the hole.
- [x] Cover three cases in that test: a window too narrow for its name at a
...
- [x] In each case assert that every `markPoint` datum's anchor value equals
      ...
      beneath it as well.
- [x] Assert the placement fields that put the two labels on that line: the
...
- [x] Run the test against the unchanged module and record that it fails on the
      anchor assertion — not on a missing property, a thrown error, or a
      formatter mismatch.

## 2. Anchor the parked label on the plot

- [x] Replace the `LABEL_Y` constant in
...
- [x] Apply the same anchor to the wrapped window's `CONTINUES` marker, which
...
- [x] Seat the parked label on that same line. `position: labelSide` centres the
      ...
      records.
- [x] Leave the fit/no-fit decision, the tail-shedding order, the chosen side,
...
- [x] Update the reserved-band comment so it describes the anchor the module now
      uses.

## 3. Verify

- [x] Run the repository's six-command fast gate and confirm each tail:
...
- [x] Confirm the new test now passes for the same reason it failed.
- [x] Commit on the ticket branch and stop. Do not open a pull request; the sweep
