...
- [x] Add one browser case to `frontend/diagnose-workstation.browser.test.mjs`,
...
- [x] Extend the same case with the control widths 900 and 1024 x 1024, where
      the two-pane split must survive unchanged and the same rows must own their
      own hit points.
- [x] Run the case against the unchanged stylesheet and record that it fails at
...
- [x] Measure the canvas pane's own minimum content width in the running
...
- [x] Raise the Diagnose stacking breakpoint at
...
- [x] Keep Verify out of it by SPLITTING the one `.vw`-affecting rule in the
      ...
      of this change.
- [x] Carry the declarations across verbatim, including the `border-left: 0`.
...
- [x] Contain the canvas pane horizontally at every width so residual overflow
...
- [x] Change nothing else in the stylesheet: no new pane widths, no
      `var(--side)` change, no narrow-layout redesign, no touch-target or
      typography edits.

## 3. Prove it

- [x] The new case passes at all six widths, and the workstation browser leg
      reports one more passing test than its 60-test baseline with zero
      failures.
- [x] Add one assertion to the Verify replay's S7 story
      ...
      change them.
- [x] The Verify replay passes 8 of 8 stories, S7 included, with that assertion
      in place.
- [x] The fast gate is green with no browser present and no Playwright
...
- [x] Record the shipped threshold and the measurement it came from in this
      change's `design.md` under the existing `## ADR 359` heading.
- [x] Stop on the ticket branch with the work committed and the verification
