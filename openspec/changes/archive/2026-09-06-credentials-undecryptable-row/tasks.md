...
- [x] In `ciq_autotune/credentials.py`, wrap the single `decrypt` call in
...
- [x] Return from the handler immediately. Do not fall through to the `.env`
      fallback below it: that fallback is conditioned on an empty table, and an
      undecryptable row is not one.
- [x] Emit exactly one `logger.warning` from that handler, through a module-level
...
- [x] Say what the code now does in the module docstring, at the sentence that
      already promises the key can be lost without a lockout.
- [x] Leave `save_credentials`, `_fernet`, the `.env` fallback's own logic, and
      both callers (`ciq_autotune/api.py`, `ciq_autotune/sync.py`) unchanged.

## 2. Pin the behavior through the public interface

- [x] `tests/test_credentials.py`: a row saved under one key and read after the
...
- [x] `tests/test_credentials.py`: with usable `tconnectsync.secret` values
...
- [x] `tests/test_api.py`: `GET /api/credentials`, on an app whose `key_path`
      cannot decrypt the stored row, answers `200` with `configured` false and a
      null `email`. This test fails first, as a `500`.
- [x] Build every fixture from the module's own `save_credentials` and a locally
...
- [x] In `openspec/specs/credentials/spec.md`, under the existing requirement
      *Losing the encryption key means re-entering credentials, not losing
      ...
      re-entry to replace.
- [x] Rewrite that requirement's existing `#### Scenario: Losing the encryption
...
- [x] Change no other requirement, and no other specification.

## 4. Leave the already-correct documents alone

- [x] `README.md`, `AGENTS.md`, `docker-compose.yml` and
