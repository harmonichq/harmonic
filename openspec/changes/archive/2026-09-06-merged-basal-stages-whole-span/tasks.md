...
- [x] Fail first: extend `frontend/diagnose-workspaces.test.js` so a merged
...
- [x] Let `stageItemsFor` accept the finding's served member start-minutes and,
...
- [x] Keep the existing guards passing untouched: a non-asserting slot stages
      nothing, an unstamped verdict stages nothing, and the stale aggregate
      `basal_rate` key stages nothing.
- [x] Cover a merged run whose members do not all assert, and prove only the
      asserting members map into the Plan.

## 2. Make the Diagnose surface act and speak for the whole run

- [x] Resolve a basal lane cell's owning finding from the live findings rows —
...
- [x] Compute the eligible member set ONCE from that owning row's served
...
- [x] State the finding's span in the panel's reserved scope line whenever the
...
- [x] Print the dock's number pair only where every staged slot agrees on it, and
...
- [x] Leave a single-slot finding's panel, dock line and staged set byte-identical
      to today.

## 3. Record and prove the revision

- [x] Satisfy this change's `surfaces` spec delta end to end — every scenario in
...
- [x] Collect base and revision renders of the merged-row panel and dock at the
...
- [x] Run the full six-command fast gate and the two browser legs this change
