...
- [x] 1.1 `Store.save_follow_up_record` accepts an available Plan reconciliation
...
- [x] 1.2 Implement plan **Only the newest recorded Plan can be pending** in
  `watched_change.pending_plan`. The apply and withdraw routes and
  `follow_up_admission` keep reading that one predicate.
- [x] 1.3 Implement plan **The server confirms a pending Plan from a matching
...
- [x] 1.4 Implement plan **Every recorded Plan serves one verdict** as one
...
- [x] 1.5 Backend tests in `tests/test_plan_verdict.py`, through `create_app`'s
...
- [x] 1.6 `CONTEXT.md`, under **Plan**: a recorded Plan is pending until it is
...
- [x] 2.1 Implement surfaces **Changes states a Plan's phase from its served
...
- [x] 2.2 Implement surfaces **Changes keeps a recorded Plan apart from a newer
  draft** in `frontend/plan-view.js`, keeping the Decision block labels S41 reads
  ("Draft saved", "Decision recorded", "On pump").
- [x] 2.3 Unit tests in `frontend/plan-actions.test.js` (and
...
- [x] 2.4 `frontend/replay-pump.py` gains an `in-place` capture: the recorded
...
- [x] 2.5 Replay contract: C2's S42 body reads "On pump since" and reads the
...
- [x] 3.1 Implement surfaces **The watch panel carries a recorded Plan awaiting
...
- [x] 3.2 `frontend/diagnose-workstation.js` paints the watch panel with the
...
- [x] 3.3 Implement surfaces **The case-file header carries no pending-Plan
...
- [x] 3.4 Unit tests: `frontend/watched-change-dock.test.js` (a pending Plan
...
- [x] 3.5 Rewrite `frontend/desk.browser.test.mjs`'s pending-Plan test at both
...
- [x] 3.6 Replay contract: add S147 on the `pattern-near-tie` case, which serves both
