...
The named database is generated by committed
`scripts/gen_revise_e2e_db.py` from fixed seed 620. It contains 30 manufactured
days, a synthetic pump profile, no credentials, and a
`synthetic_fixture_provenance` stamp. The generator's `--check`, SQLite's
integrity check and the public contamination scan all pass. A supervised run
showed schema version 8, the 2020-02-03 through 2020-03-03 synthetic range,
`configured: false`, and null fetch attempt/success state; its request log
contained only localhost reads before clean shutdown.

**Base and replay.** The revision starts at exact `origin/main`
`eda5cdfd318d638503defdd6f364a078042fe592`. The replay opens the real HTML,
CSS and JavaScript through the declared server; its named API reads are
deterministically fulfilled from the existing fixed-seed
`mockups/diagnose-workstation.synthetic/payload.json`. It has no mock opener or
second surface route. All 29 exported stories passed. The deliberately induced
D3 failure is accepted only as an exact `/timeline` HTTP 500; every other HTTP,
page or console error fails the run.

**Convergent revision.** Remove the redundant chevron from each evidence row.
The whole row remains a button and keeps opening the occurrence level. This
applies P48's existing operator sanction verbatim:

`Connor Griffin · 2026-08-19 · "Decided by Connor Griffin in a ruling session on 2026-08-19."`

The new absence story failed against the base with five chevrons, then passed
after the renderer/style retirement while also proving the occurrence drill
still works. Live before/after renders used the generated SQLite database and
cover the full-day Late bolus evidence state in explicit Light and Dark themes:
four rows and four chevrons before, four rows and zero chevrons after, with no
console errors. The screenshots remain external review evidence under
...
## ADR 179 — A drill derives no canvas window

**Decision.** Opening a finding derives no canvas window. Whatever reader-set
preset or drawn window stands continues to stand, and the same control continues
to report it. A drawn window keeps its clear affordance; a preset remains pressed
instead of being replaced by a follow chip.

**Context.** Frozen P17 and P20 were both kept under Connor Griffin's 2026-08-19
ruling, but they contradicted each other. P17 says a reader's explicit window is
a workspace that survives drilling. P20 says the same drill seizes the factor's
peak window. On 2026-08-25 Connor settled the conflict: “it just keeps my
selector. The selection is a slicing method that lets me then dig into findings.
Those findings will show up as dots on the chart anyway that I can then trace
into.” P17 is the precedence rule and P20 is permanently retired with that
sanction in the frozen behavior ledger.

**Consequences.** The factor's peak remains visible in the inspector's “When it
lands” histogram and in the coincidence routes to its basal slot and I:C block;
occurrence dots remain on the standing canvas window. Removing the factor branch
also removes its scoped arm: a sub-day server window keeps the same range but is
reported by the reader's pressed preset rather than a `Window HH:MM–HH:MM` chip.
The follow chip's shipped width is retained as deliberate slack, so this decision
changes state precedence without changing layout.

**Safe revision evidence.** `AGENTS.md` declares the sole offline entrypoint as
`uv run harmonic serve --no-fetch --db mockups/revise-e2e.synthetic/harmonic.sqlite`.
The database is generated by `scripts/gen_revise_e2e_db.py` from fixed seed 620,
has explicit synthetic provenance, and contains no real patient data. The replay's
deterministic API reads use the separately generated synthetic
`mockups/diagnose-workstation.synthetic/payload.json`.

Decision: harmonichq/harmonic#179, 2026-08-25.
