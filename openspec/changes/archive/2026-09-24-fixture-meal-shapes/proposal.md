...
- In every scoped window the browser checks request, the fixture findings mirror
  serves the server's own Pattern list and the Pattern mirror the server's own scoped
  case files, both frozen; any other scoped request fails by name. Today the fixture
  queue shows no Pattern at all in a narrowed window.
- The test desk projects the server's own inputs (the analysis, scenarios and
  exposures its Pattern lists come from), so its queue equals the server's byte for
  byte, order included; today it prices Patterns from one input and every other
  row from another.
