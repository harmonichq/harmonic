...
Diagnose read records it with one status read answered before its payload
reads are issued (no payload carries it; `input_data_age` is attached only to
a labelled stale predecessor; the recorded revision is then at or before every
payload snapshot, so a write landing during the reads always moves the
revision the next return compares), and one status read on return is the only request a retained
