...
At every viewport where Diagnose forms its two-pane split, the basal lane's
head row (its key) and every one of its cells SHALL be fully visible inside the
canvas pane, or reachable by scrolling the canvas pane alone. The lane SHALL
never be clipped out of reach, and reaching it SHALL never scroll the document.
At the two supported desktop sizes, 1280×720 and 1440×900, the lane SHALL stand
wholly inside the canvas pane at rest, and the canvas pane SHALL have no scroll
range. On a short window, every slot the key counts as raise or lower SHALL be
selectable with the pointer without resizing the window, and selecting it SHALL
open its panel with its Stage change control. The Spotlight, the glucose
overview and its clock-window gestures, All charts and fullscreen SHALL keep
their behavior. No staging, verdict or ranking decision moves.
...
- **THEN** the lane key and every cell lie inside the canvas pane's visible box,
  either at rest or after the reader scrolls the canvas pane
- **AND** the document does not scroll and no other container moves
- **AND** each raise and lower cell, pointed at where the reader sees it, opens
  its panel with a Recommended value and a Stage change control
...
- **THEN** the lane key and every cell lie wholly inside the canvas pane
