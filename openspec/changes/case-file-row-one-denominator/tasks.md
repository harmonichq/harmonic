...
- [x] 1. Add a fail-first regression through `finding_case_file.wrap`'s public
...
- [x] 2. In `wrap`, build the anchored appearance from the prepared case
      ...
      payload untouched.
- [x] 3. Add `headline` to the changed-field allowlist in
...
- [x] 4. Regenerate the committed synthetic Diagnose capture with
...
- [x] 5. Export the mirror's existing `findingHeadline` from
...
- [x] 7. Add executable `C62` to that replay — a two-family rendered row keeps
...
- [x] 8. Run the verification gate to the measured expectations in the order,
      with zero failures.
