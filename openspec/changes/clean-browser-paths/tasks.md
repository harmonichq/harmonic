...
- [ ] Add failing route-table coverage proving supported pages serve HTML, every
      ...
      swallowed.
- [ ] Bind the browser-owned page identities to the server's explicit SPA-route
...
- [ ] Replace hash-router expectations with clean path/query expectations while
      preserving migration, Back/Forward, request regeneration, and stale-response
      rejection coverage.

## 2. Separate the server namespaces

- [ ] Serve the SPA index at `/` and every supported page path without shadowing
      API or asset routes.
- [ ] Prefix every programmatic operation with `/api`; serve the generated schema
...
- [ ] Move the explicit reachable-asset allowlist beneath `/assets` without exposing
      additional frontend files.

## 3. Move browser routing and transport

- [ ] Make the Vue-free router parse and serialize clean pathname/query state,
...
- [ ] Move the browser transport's endpoint paths beneath `/api` and all local HTML
      asset references beneath `/assets`.
- [ ] Move CI and container health probes to `/api/health`; move container and
      runtime credential guidance to `/api/credentials`.
- [ ] Update Diagnose, Verify, cockpit, replay, and screenshot consumers so their
      route observations and intercepted requests exercise the shipped namespaces.

## 4. Fold the public contract and verify

- [ ] Update current README, baseline OpenSpec specs, and live behavior ledgers;
...
- [ ] Materialize the public tree, check its links, and run its contamination scan.
