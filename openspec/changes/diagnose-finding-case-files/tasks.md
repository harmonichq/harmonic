...
- [ ] Add one deep preparation built inside one SQLite read snapshot. Publish an
...
- [ ] Add one identity-bearing opportunity builder for meals, sub-70 runs,
...
- [ ] Associate every attributed Lever instance to one opportunity in its declared
      Exposure, including the caused-low split's rebound High back to its source
      Low; fail closed for an unresolvable association.
- [ ] Wrap the authoritative Findings projection without forking its policy; keep
      ADR 22's future history/selection pass-through mandatory.
- [ ] Implement ADR 79's exact schemas and bounded lease/version registry, including
...
- [ ] Add the read-only HTTP route and preserve the ranked Findings endpoint's
      separate queue responsibility.
- [ ] Implement ADR 79's declared-family algebra and fixed event table for
...
- [ ] Prove the four families, `claimed < fired`, snapshot invalidation, selection,
...
- [ ] Remove the title-keyed alignment family map and the browser-side
      `(family, ep_id, t)` population join from the Finding drill path.
- [ ] Open every visible Finding through the server case-file response; render
      the response's counts, roster, selection, and clock/event projection
      without re-deriving membership or silently falling back.
- [ ] Add correction-cluster and Highs rendering to the existing event surface,
      reusing its visual and accessibility grammar.
- [ ] Amend and replay the frozen Finding-evidence behavior ledger against the
      base and revision apps using the declared no-fetch synthetic server.
- [ ] Produce the exact 64-row render manifest: 52 image paths and the 12 declared
      base-recovery N/A cells, across both viewports and themes.

## 3. Close the regression hole

- [ ] Generate production-shaped Finding and case-file responses from committed
...
- [ ] Cover nonempty meal, correction-cluster, and Highs rosters; band/denominator
      ...
      requests.
- [ ] Regenerate every affected fixture and mirror, run their drift checks, and
      keep synthetic provenance and contamination guards intact.
- [ ] Run the full fast gate, every committed drift check, and all nine browser
