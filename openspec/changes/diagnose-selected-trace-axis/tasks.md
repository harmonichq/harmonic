...
- [x] 1. Add a regression test to `frontend/diagnose-evidence-charts.test.js`
...
- [x] 2. In `frontend/diagnose-event-comparison.js`, make the non-mini branch of
...
- [x] 3. Leave `eventComparisonGlucoseValues`, `glucoseRange`, `GLUCOSE_ENVELOPE`,
  ...
  not.
- [x] 4. Confirm the already-locked neighbours stay green without being edited:
...
- [x] 5. Provision the worktree once with `uv sync --frozen --extra api --extra
...
- [x] 6. Add one paragraph to `mockups/INDEX.md` below the surface table, in the
