...
- [ ] When `action` is `'settings'` and the mount was handed a `settings`
      ...
      throwing.
- [ ] When `action` is `'retry'`, render one primary button reading **Retry**
...
- [ ] Add one test for a **non-`401`** failure: a route stub answering `500`
      ...
      itself.
- [ ] Run both new tests against the tree **before** the fix and record that they
