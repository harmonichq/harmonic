...
- [ ] Add `frontend/diagnose-load-failure.js`, a vue-free module in the shape the
...
- [ ] It returns `{ icon, title, body, detail, action }` where `icon` is
      `'lock'` or `null`, `title` and `body` are always non-empty app copy,
      `detail` is a string or `null`, and `action` is `'settings'` or `null`.
- [ ] A cause whose numeric `status` is `401` returns
      `icon: 'lock'`,
...
- [ ] Every other cause returns `icon: null`,
      `title: "Diagnose couldn't read this server's evidence"`,
      `body: "The evidence request failed before Diagnose could read it."`,
      `action: null`, and `detail` set to the cause's own message — the string
      itself when the cause is a string, `cause.message` when it is an `Error` —
      or `null` when there is no non-empty message.
- [ ] Classify on `status` only. Never match on the wording of the server's
      `detail` string, and never return a server-authored string as `title` or
      `body`.
- [ ] Add `frontend/diagnose-load-failure.test.js` (Node's built-in runner,
      ...
      with the server's message as `detail` and no action; a plain string cause
      (`showError`'s two internal callers pass one) returns the generic copy with
      that string as `detail`; a cause with no message returns `detail: null`;
      and no returned `title` or `body` ever equals the server's message.

## 2. Render that composition inside the surface

- [ ] In `frontend/diagnose-workstation.js`, `showError` (currently
...
- [ ] Keep `root.className = 'dw dw-error'` exactly as it is. `.ec-error.dw-error`
      ...
      both depend on it.
- [ ] Keep the teardown that already opens `showError` — the abort, and clearing
      `teardown`, `repaint` and `leaveSurface` — unchanged and ahead of the render.
- [ ] The rendered block follows the missing-token placeholder's pattern at
      `frontend/index.html:1596-1602`: an optional icon, an `<h2>` with `title`, a
      `<p>` with `body`, an optional detail line, and an optional primary button.
      Carry `role="status"` on the block, as that placeholder does.
- [ ] When `icon` is `'lock'`, use the lock `<svg>` markup from that placeholder
      verbatim. Invent no new icon for any other case; `icon: null` renders none.
- [ ] When `action` is `'settings'` and the mount was handed a `settings`
      ...
      throwing.
- [ ] Add the `.dw-error` rule that has never existed, in
...
- [ ] Leave `render()`, `boot()`, the payload guard and every other `showError`
...
- [ ] In `frontend/index.html`, `loadAudit`'s catch passes the caught error object
      ...
      in that catch moves.
- [ ] In `diagnoseEnsureView`, add one `settings` callback to the callbacks object
      already carrying `stage`, `isStaged`, `day` and `retry`, calling the app's
      existing `setTab('settings')`.
- [ ] Change no other screen. The shell's `!hasToken` placeholders, the topbar's
      *No API token set* banner, `frontend/verify-workstation.js`'s own
      ...
      are.
- [ ] Change nothing server-side. `ciq_autotune/api.py` keeps answering `401` with
      its own `detail`; this change only decides how Diagnose reads it.

## 4. Prove the rendered state in the browser

- [ ] In `frontend/diagnose-workstation.browser.test.mjs`, update the existing
      ...
      textContent.
- [ ] Keep the existing `#654` first-load-failure regression passing unchanged in
      intent: `.dw.dw-error` still appears, the surface still shows a failure
      message, and no uncaught error reaches the page.
- [ ] Add one test for the reported state: a route stub answering `401` with
...
- [ ] Run that new test against the tree **before** the fix and record that it
      fails for the right reason — the block's heading is the server's own
      `missing or invalid bearer token` and there is no control inside
      `.dw.dw-error` — not because a selector or a route stub is wrong. A test that
      goes green without ever having been red proves nothing here.
- [ ] No test reads or asserts a token value, and no rendered path reads
      `localStorage.ciq_token`.

## 5. Record the revision on the frozen surface

- [ ] Append one dated amendment section to
      ...
      Diagnose load-failure state now shows and that a rejected token routes to
      Settings. It amends no existing story and retires nothing, so it issues no
      new executable ID and needs no retirement sanction.
...
- [ ] `frontend/data.js`, `ciq_autotune/`, the findings projection, the analyzers
      ...
      support floor.
- [ ] The dead `.ec-error` rule in `frontend/diagnose-event-comparison.css` is not
