...
- [x] Make case-file preparation answer a lever-and-window request, not only a
      Finding that fired.
- [x] Scope the attribution equations to Finding-keyed requests, so a
      lever-and-window request is not measured against a claim it does not carry.
- [x] Retire the standalone comparison projection, its vocabulary, its request
      coordinates, and its schema; move or retire every fixture, mirror and
      drift check that fed it.

## 2. Build the one comparison

- [x] Assemble the three partitioning cohorts per the ADR's identity table.
- [x] Take out only the Occurrences this lever matched; leave in the ones
      another lever claimed.
- [x] Anchor both cross-Exposure pairs on a meal dose per the ADR.
- [x] Serve the cohort counts, the not-comparable count, and the cohort names.
- [x] Serve the state a Withheld comparison population produces: the matched line
      alone, saying the comparison is unavailable.
- [x] Cover each lever through the public interface: partition, counts
...
- [x] Render the three lines plus the reader's selected Occurrence trace, on the
      served names and counts.
- [x] Amend the frozen behavior ledger and replay it against the built app.
- [x] Regenerate every affected synthetic capture through its generator, with
      each `--check` preserved.

## 4. Fold into the baseline

- [x] Update the two capability statements that assert five event cohorts, in
