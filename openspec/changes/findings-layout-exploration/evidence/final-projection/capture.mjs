...
for (const viewport of [{ width: 390, height: 844 }, { width: 360, height: 800 }, { width: 760, height: 900 }, { width: 1024, height: 768 }, { width: 2084, height: 742 }, { width: 1440, height: 900 }]) {
  const size = `${viewport.width}x${viewport.height}`;
  const page = await openApp(browser, {
    state: 'typical', viewport, history: true, hasTouch: viewport.width <= 480, isMobile: viewport.width <= 480,
  ...
  await shot(page, size, 'drill-return');
  ...
  await shot(page, size, 'all-charts-dismissed');
  ...
  await page.getByRole('button', { name: 'Close', exact: true }).click();
  await shot(page, size, 'fullscreen-return');
