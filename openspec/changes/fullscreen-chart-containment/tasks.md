...
- [ ] Run the frozen workstation and event-comparison replays against the safe
      synthetic app source and inventory mount, resize, theme, dismissal, and
      disposal behavior.
- [ ] Add a fail-first browser assertion for the committed Carb undercount case at
      2084×450, proving the current frame escape and plot/key overlap.
- [ ] Establish the four-family matrix at 2084×450 and 2084×742 before production
      edits.

## 2. Centralize the fullscreen frame

- [ ] Make the workstation own fullscreen bounds, overflow, resize observation,
      theme repaint, replacement, disposal, and dismissal restoration.
- [ ] Return an observer-free mount record from `renderEventSurface`; retain
      caller-owned observation for any real non-fullscreen compatibility caller.
- [ ] Remove event-comparison-specific fullscreen height and overflow authority
      without adding a parallel composition or speculative module.

## 3. Preserve chart behavior and restoration

- [ ] Keep basal, ISF, and carb-ratio registry options unchanged.
- [ ] Preserve the response comparison's served cohort key, selected occurrence,
      selected trace/mark, accessible label, and Left/Right keyboard cursor.
- [ ] Prove Back/Escape restores the exact prior Spotlight, chart dock, pins,
      selection, and arrangement for every family.

## 4. Record and verify the revision

- [ ] Amend `mockups/finding-evidence-routing.behavior.md` and `mockups/INDEX.md`.
- [ ] Capture synthetic same-fixture Light/Dark before-and-after evidence for all
      four families at the red and control viewports.
- [ ] Run the complete fast gate and drift checks plus the affected workstation,
