...
- [ ] 2.5 In [#390](https://github.com/harmonichq/harmonic/issues/390), reshape the first release so Diagnose leads with outcome shapes and v2 ships three destinations (Diagnose, Changes, Day).
  - [ ] 2.5.1 [#391](https://github.com/harmonichq/harmonic/issues/391) spike: settle the shape unit (membership, rate/denominator, impact rule, collapse, subject, opportunity-gated Focus readiness, near-tie); ADR 391 records; read-only aggregate snapshot pass; the unit is named pattern (Connor, 2026-09-08).
  - [ ] 2.5.2 Backend build (unfiled; filed after 2.5.1 is admitted): shapes in scenario report / findings projection / guidance selection, Focus pin on a shape, opportunity-gated readiness; existing verdicts, floors, classifiers and scores unchanged as inputs. Begins with the synthetic replay of the ADR 391 rulings over the generator-owned QA catalog through public producers, pinned by tests (re-sliced from 2.5.1 on 2026-09-08).
  ...
  - [ ] 2.5.4 Design child (unfiled): per-shape chart and shape row on the shipped rail; `/ui-craft` revise of the shipped Diagnose surface; Connor holds the lock.
