...
Row pointer and keyboard activation, All charts entry, selected occurrence identity, Clear trace, fullscreen return/focus, clock selection and existing typed recovery SHALL survive the revision. The current ability to hide and restore either cohort through its legend SHALL remain available for High-carb sequence, with a keyboard-operable control. Hiding a curve SHALL not change the underlying comparison, counts or summary. Selecting an eligible roster occurrence SHALL expose and overlay only that occurrence's observed glucose within the same response interval and anchor. The original sequence metadata SHALL remain available. Unknown or out-of-roster selections SHALL retain the existing unavailable-selection result. Sequence detail SHALL NOT add a Day handoff.
