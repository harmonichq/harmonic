...
- **Freeze rule (release).** No `★ FROZEN` block is rewritten, re-dated or
  ...
  and moves only the numeric literals in
  `mockups/sweep/harmonic-v2-desktop/acceptance.py` and its test so its own tests
  pass. `ACCEPTANCE.md`'s count sentence, `mockups/INDEX.md` and the one release
  freeze block are the release coordinator's, written once on the integration
  branch.
