...
after the low's nadir and at or before the guarded rebound's terminal. "Fired"
...
Meal bolus fell short (the two split one rise population). Its retained verdicts
for both are non-matches with silence reason `upstream_cause` and evidence tier
Inferred. Ownership is consulted after the rise checks and after the context
gate. A rise the gate already explains keeps its gate verdict byte-for-byte,
because committed fixtures freeze that text. Otherwise the detail names the
owning low's nadir value and time. An owned High contributes no candidate, so no
missed-meal or meal-bolus-short impact price counts it. An Episode holding only
owned Highs draws no Lever, and its silence reason is that missed-meal verdict,
...
reaches the later of that terminal and the end of every High run it owns. That
...
`uncaused` tally skips them. This refines ADR 63 Decision 6; the count stays
Episode-wise for every other High. An owned High stays a non-driver Occurrence
