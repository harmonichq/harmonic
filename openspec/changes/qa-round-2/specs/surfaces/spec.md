...
direction and values. When they name none and no stage save the surface issued
is in flight, the dock SHALL name the draft from its own items: the setting in the wearer's words and the span the items cover,
...
#### Scenario: The draft read before a press is not reported while its save is in flight

- **GIVEN** a served Plan draft holding one basal row, staged on the surface
...
painted, and SHALL ask it again once an accepted stage save settles. It SHALL derive no staging eligibility of its own: a mark follows the
