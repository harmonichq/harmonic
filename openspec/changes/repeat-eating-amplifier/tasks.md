...
- [ ] 1. Populate `repeat_eating_amplifier` in `build_report` through the shared
  quintile assignment and eligibility pass, carrying the same exclusions as the
  high-carb detector.
- [ ] 2. Add the adverse finding condition, two-tier headline selection, and fixed
  TIR and glucose-spread templates for matched `3+`-versus-`1` comparisons.
- [ ] 3. Extend the synthetic stream builder for multi-window sequences and
...
- [ ] 4. Add public-interface tests for every evidence anchor in the proposal.
