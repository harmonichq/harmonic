...
- [ ] Replay and inventory the Cockpit shell at exact ticket base
      `b075c715a497b55e684f966cf046dc9179f428ab` before changing production
      source.
- [ ] Re-freeze `mockups/cockpit-shell.behavior.md` with hashes computed from
      every exact synthetic source its opener transports.
- [ ] Add permanent retired entry R1 with owner Connor, settlement date
...
- [ ] Add a closed source-inventory assertion over `frontend/index.html` for
...
- [ ] Extend only the Cockpit test adapter with optional initial-hash, findings-
...
- [ ] Export and register R1 in `COCKPIT_SHELL_STORIES` beside the adjacent
...
- [ ] Cold-open a stale URL whose identifier comes from the generated fixture's
...
- [ ] Use the existing `proveRedOnce` seam to make the canonical-hash and no-
...
- [ ] Delete the legacy ref, helpers, hash serialization/restoration, analysis
      retry, watcher, Escape branch, setup exposure, and obsolete comments from
      `frontend/index.html`.
- [ ] Keep the shared hash mechanism, `goToMoment` Day handoff, and all non-
      occurrence branches—including Data quality—unchanged. Do not repair the
      separate direct-link parsing gap tracked by #53.
- [ ] Rerun the source-inventory assertion and retain its green result.

## 4. Verify and review

- [ ] Capture base and revision stale-link/visible-Inspector evidence at
...
- [ ] Record the source test's red/green evidence, both restored browser
      mutations, the nonzero Cockpit story count, and the full R1 sanction in
      `evidence/review.md`.
- [ ] Run the complete fast gate and current drift checks, then the Diagnose
