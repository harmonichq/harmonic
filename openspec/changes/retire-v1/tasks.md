...
- [x] 1.1 Rehome the desk's lifted material, chart key and glossary as committed
...
- [x] 1.2 Re-point the three lifts in `mockups/harmonic-v2.exploration/generate.py`
...
- [x] 1.3 Copy out of the two v1 replay files every export the surviving tree
      ...
      v1 ledger stories.
- [x] 1.4 Rewrite `tests/test_frontend_asset_routes.py` first. It asserts the
      ...
      then make it pass.
- [x] 1.5 In `ciq_autotune/api.py`, serve the desk's built shell at `/` and the
...
- [x] 1.6 Set the desk build's base to `/`. Move the browser router's desk prefix
...
- [x] 1.7 In `frontend/browser-gates-fail-closed.test.js`, the suite list becomes
...
- [x] 1.8 Re-point every desk test, browser suite and replay opener from `/v2/...`
...
- [x] 1.9 Delete the v1-against-desk comparison driver
      `mockups/sweep/harmonic-v2-desktop/clinical-pairs.mjs` and its two tests in
      `frontend-v2/c4.replay.test.js`.
- [x] 1.10 Update the acceptance driver's package proof and its test to the root
...
- [x] 1.11 Group gate: `npm ci && npm run build`; `uv run python -m pytest
...
- [x] 2.1 Delete the v1 shell page, the v1 Vite config, the two v1 replay files,
...
- [x] 2.2 Delete these nine browser legs from `.github/workflows/ci.yml`, by gate
      ...
      jobs.
- [x] 2.3 Retire the two generators whose fixture has no surviving reader, with
...
- [x] 2.4 Delete `harness/`, `frontend/harness-api-paths.test.js`, and the three
...
- [x] 2.5 Re-point `scripts/check_owned_identifiers.py`: the browser title rule at
...
- [x] 2.6 Group gate: `npm ci && npm run build`, the fast-gate node line, full
...
- [x] 3.1 Merge `frontend-v2/` into `frontend/` and apply every rename in
...
- [x] 3.2 Rename router, server and CI identifiers per the naming boundary.
- [x] 3.3 Reconcile the living documents to one shell at root: `AGENTS.md`
...
- [x] 4.1 On the commit to be pushed, build once and run the desk suite, the
      follow-up suite and the browser-runner regression. Fold every correction
      back and re-run only what the correction touches.
- [x] 4.2 Run the complete desk ledger once at each accepted size against the
