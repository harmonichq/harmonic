...
## ADR 451 — The carb-ratio analyzer's sentences say carb ratio

**Decision.** Every sentence the carb-ratio analyzer serves
(`ciq_autotune/analyzers/ic.py`) names the setting "carb ratio" instead of
"I:C":

- `_recommend`'s estimate reasons and recommendation annotations;
- the hold annotations near the prior-meal-action gate, and the "Read with the …
  stretch" prefix that wraps them;
- the carb-counting finding's summary and occurrence detail.

Comments, docstrings and error messages keep the engine term.

These copies are regenerated or re-dumped, never hand-edited:

- `frontend/__fixtures__/analysis.json` and the findings-projection fixture,
  each regenerated by its generator;
- the QA queue-row literals and `tests/test_analyzer_ic.py`'s pinned line,
  re-dumped by the AGENTS.md coverage-era process.

**Guard.** A new test in `tests/test_annotation_register.py` builds every served
carb-ratio sentence branch and asserts that none contains "I:C" or "ISF". It
applies the register's engine-name rules only. The carb-ratio sentences also
use prose em dashes and "clean-start", which two other register rules ban; that
was reported to the release coordinator rather than widened here.
...
- **Tests pinning the old titles and sentences.** Move these with the titles:
  ...
  - `tests/test_analyzer_ic.py`'s "direction needs more identifiable meals"
    line;
