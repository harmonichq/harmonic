...
- [x] Record the base: the ticket branch's merge-base with `origin/main`. Spin a
...
- [x] Append a ledger provenance amendment. The ledger's head and its #83
...
- [x] Amend `mockups/finding-evidence-routing.behavior.md` and the replay in
...
- [x] Attended-free revise round on the hero's card treatment, type ranks,
...
- [x] Evidence under `openspec/changes/tapered-urgency-queue/evidence/`: the
