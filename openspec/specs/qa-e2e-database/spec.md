...
### Requirement: Dedicated sequence fixtures preserve served transport

The dedicated payload SHALL contain fifteen manufactured states with whole-day
(global) and Overnight (0-360) preparations. It SHALL retain original producer
findings.rows, rendered_rows, event/clock cases and every sequence selection.
Repeated containers MAY be interned through a shared table and $ref entries;
one fixture-only decoder SHALL expand the original values without reconstructing
judgments or findings. Expanded payloads SHALL compare equal to fresh public
producer output and the committed payload SHALL remain below 1,000,000 bytes.
The twelve QA recipes SHALL cover each lever's covered, empty, thin-candidate,
thin-reference, losing and multiple states. Projection parity SHALL additionally
cover missing, null and midnight witnesses.

#### Scenario: Interning does not alter a served preparation
- **WHEN** the dedicated capture is decoded for the harness, replay or Node tests
- **THEN** both windows retain the original public preparation and all sequence selections
- **AND** equality and size checks pass without rebuilding a verdict or findings row
