...
def test_pattern_case_tags_meal_bolus_short_on_the_meal_it_claims():
    meal_at = datetime(2026, 8, 1, 12, 30)
    findings = _pattern_findings(
        "highs_after_meals", (Lever.CARB_UNDERCOUNT,), k=1, n=1,
        rate_levers=(Lever.CARB_UNDERCOUNT, Lever.LATE_BOLUS, Lever.MEAL_BOLUS_SHORT),
    )
    prepared = _prepared(
        Lever.CARB_UNDERCOUNT, findings=findings,
        exposures={"exposures": {
            "meals": {"n": 1, "occurrences": [{
                "ep_id": "meal", "t": meal_at.strftime("%Y-%m-%d %H:%M:%S"),
                "kind": "meal", "bg": 120, "attributed": False,
                "cause_lever": None, "verdicts": [],
            }]},
            "highs": {"n": 1, "occurrences": [{
                "ep_id": "short", "t": "2026-08-01 14:00:00",
                "kind": "high", "bg": 230, "attributed": True,
                "cause_lever": "meal_bolus_short", "verdicts": [],
            }]},
        }},
        scenarios={"episodes": {"short": {"lever": "meal_bolus_short", "steps": [{
            "citation": {"facts": {
                "meal_at": meal_at.strftime("%Y-%m-%d %H:%M:%S"),
            }},
        }]}}},
    )
    ...
    assert case["summary"] == {"claimed": 1, "denominator": 1, "noun": "meals"}
    assert case["occurrences"][0]["member"] == "habit:meal_bolus_short"


def test_pattern_case_reads_stacked_and_on_iob_corrections_over_lows():
    first_low = datetime(2026, 8, 1, 12, 30)
    second_low = first_low + timedelta(hours=4)
    findings = _pattern_findings(
        "lows_after_correcting_highs",
        (Lever.CORRECTION_STACKING, Lever.CORRECTION_ON_IOB), k=2, n=2,
        rate_levers=(Lever.CORRECTION_STACKING, Lever.CORRECTION_ON_IOB),
    )
    prepared = _prepared(
        Lever.CORRECTION_ON_IOB, findings=findings,
        exposures={"exposures": {
            "lows": {"n": 2, "occurrences": [{
                "ep_id": "stacked-low", "t": first_low.strftime("%Y-%m-%d %H:%M:%S"),
                "kind": "low", "bg": 58, "attributed": False,
                "cause_lever": None, "verdicts": [],
            }, {
                "ep_id": "on-iob", "t": second_low.strftime("%Y-%m-%d %H:%M:%S"),
                "kind": "low", "bg": 61, "attributed": True,
                "cause_lever": "correction_on_iob", "verdicts": [],
            }]},
            "correction_clusters": {"n": 7, "occurrences": [{
                "ep_id": "stacked", "t": "2026-08-01 12:00:00",
                "kind": "correction", "bg": 210, "attributed": True,
                "cause_lever": "correction_stacking", "verdicts": [],
            }]},
        }},
        scenarios={"episodes": {"stacked": {"lever": "correction_stacking", "steps": [{
            "citation": {"facts": {
                "nadir_at": first_low.strftime("%Y-%m-%d %H:%M:%S"),
            }},
        }]}}},
    )

    case = prepared.case("pattern:lows_after_correcting_highs", "event", None)

    assert case["family"] == case["population"] == "lows"
    assert case["summary"] == {"claimed": 2, "denominator": 2, "noun": "lows"}
    assert [row["member"] for row in case["occurrences"]] == [
        "habit:correction_stacking", "habit:correction_on_iob",
    ]
    assert case["projection"]["window_min"] == [-60, 120]
